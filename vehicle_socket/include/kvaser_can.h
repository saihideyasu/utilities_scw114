...
public:
	...
	canStatus initSerial(unsigned int serial, int canBitrate)
	{
		int chanCount;
  		canStatus stat = canGetNumberOfChannels(&chanCount);
		if (stat != canOK) return stat;
std::cout <<"ch : " << chanCount << std::endl;
		for(int chan=0; chan<chanCount; chan++)
		{
			unsigned int ser[2];
			stat = canGetChannelData(chan, canCHANNELDATA_CARD_SERIAL_NO, &ser, sizeof(ser));
			std::cout << "stat :" << stat << "ser :" << ser[0] << std::endl;
			if (stat == canOK)
			{
				if(serial == ser[0]) return initChannel(chan, canBitrate);
			}
		}

		return canStatus::canERR_NOCHANNELS;
	}

	canStatus initChannel(int kvaser_channel, int canBitrate)
	{
