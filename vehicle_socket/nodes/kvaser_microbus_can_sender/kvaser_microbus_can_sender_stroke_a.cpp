...
private:
	...
	double cmd_steer_diff_, cmd_steer_diff_acc_;
	double stop_distance_over_add_, stop_distance_over_sum_;
	...
	int stopper_distance_flag_;
	int use_stopper_distance_val_;
	double stopper_distance_fixed_velocity_;
	int stopper_distance_send_process_;
	...
	void callbackMicrobusCan503(const autoware_can_msgs::MicroBusCan503::ConstPtr &msg)
	{
		...
		if(msg->clutch==true && can_receive_503_.clutch==false)
		{
			...
			pid_params.set_stroke_prev(can_receive_503_.pedal_displacement);
			std::string safety_error_message = "";
			publishStatus(safety_error_message);
		}
						   ...
						   const geometry_msgs::PoseStampedConstPtr &pose_msg)
	{
		...
		double tire_angle;
		if(twist_.ctrl_cmd.steering_angle > 0) tire_angle = twist_.ctrl_cmd.steering_angle*wheelrad_to_steering_can_value_left + wheelrad_to_steering_can_value_left_intercept;
		else tire_angle = twist_.ctrl_cmd.steering_angle*wheelrad_to_steering_can_value_right + wheelrad_to_steering_can_value_right_intercept;
...
//		str << x << "," << v0 << "," << v << "," << v_sa;
		...
		str <<  "|" << use_stopper_distance_val_;
		name << "|" << "use_stopper_distance_val";
		str <<  "|" << stopper_distance_fixed_velocity_;
		name << "|" << "stopper_distance_fixed_velocity";
		str <<  "|" << stopper_distance_send_process_;
		name << "|" << "stopper_distance_send_process";
		str <<  "|" << cruse_velocity_;
		name <<  "|" << "cruse_velocity";
	...
	void callbackTwistCmd(const autoware_msgs::VehicleCmd::ConstPtr &msg)
	{
		...
		ros::Duration rostime_diff = msg->header.stamp - twist_.header.stamp;
		...
		cmd_steer_diff_ = msg->ctrl_cmd.steering_angle - twist_.ctrl_cmd.steering_angle * ws_ave * 720.0 / 15000.0;
		...
		if(cmd_steer_diff_acc_ < deg_th_left || cmd_steer_diff_acc_ > deg_th_right)// && strinf.mode == MODE_PROGRAM)
		{
			if(msg->ctrl_cmd.steering_angle != 0 && cmd_steer_diff_acc_ < 10000000 && cmd_steer_diff_acc_ > -10000000)
			{
				flag = true;
				angle_limit_over_ = true;
				std::cout << "Denger! target angle over : " << cmd_steer_diff_ << "," << cmd_steer_diff_acc_ << "," << time_diff << std::endl;
			}
		...
		if(flag == true && can_receive_502_.clutch == true && use_safety_localizer_ == true)
		{
			...
			//flag_steer_mode_ = false;
			shift_auto_ = false;
			safety_error_message << "target angle over , " << cmd_steer_diff_ << "," << cmd_steer_diff_acc_;
	...
	void callbackWaypoints(const autoware_msgs::Lane::ConstPtr &msg)
	{
		...
		/*char buf[100];
		sprintf(buf, "%lf", lane_pitch_ * 180 / M_PI);
		std_msgs::String str;
		str.data = buf;
	...
	void bufset_steer(unsigned char *buf)
	{
		...
		if(input_steer_mode_ == false)
		{
			...
			if(wheel_ang > 0)
			{
				steer_val = wheel_ang * wheelrad_to_steering_can_value_left + wheelrad_to_steering_can_value_left_intercept + steer_correction_;
			}
			else
			{
				steer_val = wheel_ang * wheelrad_to_steering_can_value_right + wheelrad_to_steering_can_value_right_intercept + steer_correction_;
			}
...
//P
	...
	bool stopper_distance_accele_checker(double accel_mode_avoidance_distance)
	{
		switch(stopper_distance_.send_process)
		{
			...
			case autoware_msgs::StopperDistance::TEMPORARY_STOPPER:
				if(stopper_distance_.distance<0 || stopper_distance_.distance>accel_mode_avoidance_distance || stopper_distance_.fixed_velocity != 0)
					return true;
				else return false;
	...
	double _accel_stroke_pid_control(double current_velocity, double cmd_velocity)
	{
		...
		if(lane_pitch_average_ < -1.0 && ret > 250) 
		{
			
			//経路の傾きによる制限
			if(lane_pitch_ > -1.2 * M_PI / 180.0)
			{
				str_stroke_process.data = "accel pitch2";
				ret = 0;
			}
			else
			{
				str_stroke_process.data = "accel pitch1";
				ret = 200;
			}
		}
		if(stopper_distance_.distance >= 0 && ret > 250)
		{
			str_stroke_process.data = "accel stopper";
			ret = 250;//停止情報がある場合の制限 
		}
		else str_stroke_process.data = "accel";
	...
	double _brake_stroke_pid_control(double current_velocity, double cmd_velocity, double acceleration)
	{
		...
		pid_params.plus_brake_diff_sum_velocity(plus_e);
		double brake_max_i;
		...
		if (pid_params.get_brake_diff_sum_velocity() > brake_max_i)
			e_i = brake_max_i;
		else
		...
		use_stopper_distance_val_ = (use_stopper_distance_ == true) ? 1 : 0;
		stopper_distance_fixed_velocity_ = stopper_distance_.fixed_velocity;
		stopper_distance_send_process_ = stopper_distance_.send_process;
	...
	void bufset_drive(unsigned char *buf, double current_velocity, double acceleration, double stroke_speed)
	{
		...
		else
		{
			...
			std::cout << "twist.ctrl_cmd " << twist_.ctrl_cmd.linear_velocity * 3.6 << ", cmd_velocity " << cmd_velocity << std::endl;
			...
			/*char buf[100];
			bool flag1 = (checkMobileyeObstacleStop(nowtime) == false);
			bool flag2 = (fabs(cmd_velocity) > current_velocity + setting_.acceptable_velocity_variation);
			bool flag3 = (current_velocity < setting_.velocity_limit);
			bool flag4 = stopper_distance_accele_checker(accel_mode_avoidance_distance);
			bool flag5 = (in_accel_mode_ == true);
			bool flag6 = (lane_pitch_ > -1.2 * M_PI / 180.0);
			sprintf(buf, "%d,%d,%d,%d,%d,%d", (int)flag1, (int)flag2, (int)flag2, (int)flag4, (int)flag5, (int)flag6);
			std_msgs::String str;
			str.data = buf;
			pub_tmp_.publish(str);*/
			...
			if (checkMobileyeObstacleStop(nowtime) == false
					...
					&& stopper_distance_accele_checker(accel_mode_avoidance_distance)
...
public:
	...
	void can_send()
	{
		...
		//if(can_receive_501_.emergency == false)
		{
			...
			str_tmp.data = std::string((char*)buf2);
