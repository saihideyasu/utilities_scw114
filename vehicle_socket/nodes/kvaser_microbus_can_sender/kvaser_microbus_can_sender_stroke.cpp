...
private:
	...
	//error params pid_params_.get_stroke_prev();
	...
	//steer direction
	const static int STEER_DIRECTION_LEFT = 0;//ハンドルを左に切っている
	const static int STEER_DIRECTION_RIGHT = 1;//ハンドルを右に切っている
	...
	ros::Publisher pub_way_distance_ave_, pub_automode_mileage_, pub_stroke_process_, pub_tmp_;

	ros::NodeHandle nh_, private_nh_;
	ros::Subscriber sub_microbus_drive_mode_, sub_microbus_steer_mode_, sub_vehicle_cmd_;
	...
	ros::Subscriber sub_waypoint_param_, sub_waypoints_, sub_config_microbus_can_, sub_config_velocity_set_, sub_config_temporary_stopper_, sub_config_lane_rule_;
	...
	ros::Subscriber sub_log_folder_, sub_solar_direction_, sub_waypoints_file_name_, sub_sip_signal_error_;
	ros::Subscriber sub_rs232_shhv_, sub_rs232_sppm_, sub_rs232_shhv_arrange_, sub_signal_takeover_;//rs232の出力
	ros::Subscriber sub_node_dead_;
	...
	ros::Time drive_clutch_timer_, steer_clutch_timer_, run_time_;
	...
	std::string automode_mileage_write_path_;//自動走行距離を記録するファイルのパス
	std::ofstream automode_mileage_stream_;//自動走行記録ストリーム
	...
	double waypoint0_velocity_;//waypointsの最初の速度
	autoware_can_msgs::MicroBusSPPM rs232_sppm_;//rs232出力
	autoware_can_msgs::MicroBusSHHV rs232_shhv_;//rs232出力
	autoware_can_msgs::MicroBusSHHVArrange rs232_shhv_arrange_;//shhvの数値を整形したもの
	bool signal_takeover_flag_;//信号で先読み処理が発生した
	double lane_pitch_, lane_pitch_average_;//現在の経路のpitch
	bool use_distance_safety_;//距離のフェイルセーブを使用するか
	double steer_i_;//steerのi積算値
	double steer_i_own_second_value_;//steer_iの１秒間での上昇値
	double steer_i_max_;//steer_iの最大値
	int steer_direction_;//現在のハンドルを回している方向
	...
	void callbackNodeDead(const std_msgs::String::ConstPtr &msg)
	{
		if(msg->data != "")
		{
			...
			shift_auto_ = false;
			std::cout << "Node Dead : " << msg->data << std::endl;
			std::stringstream safety_error_message;
			safety_error_message << "Node Dead : " << msg->data;
	...
	void callbackSignalTakeover(const std_msgs::Bool::ConstPtr &msg)
	{
		signal_takeover_flag_ = msg->data;
	}

	void callbackRS232SPPM(const autoware_can_msgs::MicroBusSPPM::ConstPtr &msg)
	{
		rs232_sppm_ = *msg;
	}

	void callbackRS232SHHV(const autoware_can_msgs::MicroBusSHHV::ConstPtr &msg)
	{
		rs232_shhv_ = *msg;
	}

	void callbackRS232SHHVArrange(const autoware_can_msgs::MicroBusSHHVArrange::ConstPtr &msg)
	{
		rs232_shhv_arrange_ = *msg;
	}

	void callbackSipSignalError(const std_msgs::String::ConstPtr &msg)
	{
		if(msg->data != "")
		{
			if(can_receive_501_.drive_auto == autoware_can_msgs::MicroBusCan501::DRIVE_AUTO)
			...
			//flag_steer_mode_ = false;
			shift_auto_ = false;
			std::cout << "Sip Signal Errro" << std::endl;
			std::stringstream safety_error_message;
			safety_error_message << "Sip Signal Errro";
			publishStatus(safety_error_message.str());
			//system("aplay -D plughw:PCH /home/autoware/one33.wav");
			can_send();
		}
	...
	void callbackSolarDirection(const autoware_msgs::SolarDirection::ConstPtr &msg)
	{
		if(msg->direction_flag == true)
		{
			if(can_receive_501_.drive_auto == autoware_can_msgs::MicroBusCan501::DRIVE_AUTO)
				drive_clutch_ = false;
			//if(can_receive_501_.steer_auto == autoware_can_msgs::MicroBusCan501::STEER_AUTO)
			...
			//flag_steer_mode_ = false;
			shift_auto_ = false;
			std::cout << "Denger! Solar Direction : Drive Manual Mode" << std::endl;
			std::stringstream safety_error_message;
			safety_error_message << "Denger! Solar Direction\nDrive Manual Mode";
			publishStatus(safety_error_message.str());
			//system("aplay -D plughw:PCH /home/autoware/one33.wav");
			can_send();
		}
	...
	geometry_msgs::Point way_pose_sum_;
	void callbackDifferenceToWaypointDistance(const autoware_msgs::DifferenceToWaypointDistance::ConstPtr &msg)
	{
		...
		way_distance_count_++;
		way_pose_sum_.x += current_pose_.pose.position.x;
		way_pose_sum_.y += current_pose_.pose.position.y;
		way_pose_sum_.z += current_pose_.pose.position.z;
		if(use_distance_safety_ == true) waypointDistanceCheck(msg, std::string("current"));
		difference_toWaypoint_distance_ = *msg;
	}

	double way_distance_sum_ndt_ = 0;
	int way_distance_count_ndt_ = 0;
	geometry_msgs::Point way_pose_sum_ndt_;
	void callbackDifferenceToWaypointDistanceNdt(const autoware_msgs::DifferenceToWaypointDistance::ConstPtr &msg)
	{
		...
		way_pose_sum_ndt_.x += ndt_pose_.pose.position.x;
		way_pose_sum_ndt_.y += ndt_pose_.pose.position.y;
		way_pose_sum_ndt_.z += ndt_pose_.pose.position.z;
		if(config_localizer_switch_.localizer_check == 0 || config_localizer_switch_.localizer_check == 2)
			if(use_distance_safety_ == true) waypointDistanceCheck(msg, std::string("ndt"));
		difference_toWaypoint_distance_ndt_ = *msg;
	}

	double way_distance_sum_gnss_ = 0;
	int way_distance_count_gnss_ = 0;
	geometry_msgs::Point way_pose_sum_gnss_;
	void callbackDifferenceToWaypointDistanceGnss(const autoware_msgs::DifferenceToWaypointDistance::ConstPtr &msg)
	{
		...
		way_pose_sum_gnss_.x += gnss_pose_.pose.position.x;
		way_pose_sum_gnss_.y += gnss_pose_.pose.position.y;
		way_pose_sum_gnss_.z += gnss_pose_.pose.position.z;
		if(config_localizer_switch_.localizer_check == 1 || config_localizer_switch_.localizer_check == 2)
			if(use_distance_safety_ == true) waypointDistanceCheck(msg, std::string("gnss"));
		difference_toWaypoint_distance_gnss_ = *msg;
	}

	double way_distance_sum_ekf_ = 0;
	int way_distance_count_ekf_ = 0;
	geometry_msgs::Point way_pose_sum_ekf_;
	void callbackDifferenceToWaypointDistanceEkf(const autoware_msgs::DifferenceToWaypointDistance::ConstPtr &msg)
	{
		...
		way_pose_sum_ekf_.x += ekf_pose_.pose.position.x;
		way_pose_sum_ekf_.y += ekf_pose_.pose.position.y;
		way_pose_sum_ekf_.z += ekf_pose_.pose.position.z;
		//if(config_localizer_switch_.localizer_check == 1 || config_localizer_switch_.localizer_check == 2)
		//	if(use_distance_safety_ == true) waypointDistanceCheck(msg, std::string("gnss"));
	...
	void callbackGnssStandardDeviation(const autoware_msgs::GnssStandardDeviation::ConstPtr &msg)
	{
		...
		if((config_localizer_switch_.localizer_check == 1 || config_localizer_switch_.localizer_check == 2) && localizer_select_num_ == 1)
		{
			if((msg->lat_std_dev > setting_.gnss_lat_limit ||
				...
				msg->alt_std_dev > setting_.gnss_alt_limit ||
				msg->azimuth_std_dev > setting_.gnss_yaw_limit && localizer_select_num_ == LOCALIZER_SELECT_GNSS) &&
				use_safety_localizer_ == true)
			{
				...
				//flag_steer_mode_ = false;
				shift_auto_ = false;
				std::cout << "Denger! Gnss deviation limit over : " << msg->lat_std_dev << "," << msg->lon_std_dev << "," << msg->alt_std_dev << "," << msg->azimuth_std_dev << std::endl;
				std::stringstream safety_error_message;
	...
	void callbackMicrobusCan502(const autoware_can_msgs::MicroBusCan502::ConstPtr &msg)
	{
		...
		if(msg->MLD == true)
		{
			...
			//flag_steer_mode_ = false;
			shift_auto_ = false;
			std::cout << "Denger! Steer Motor wire Expire" << std::endl;
			std::stringstream safety_error_message;
			safety_error_message << "Denger! Steer Motor wire Expire";
			publishStatus(safety_error_message.str());
			can_send();
		}
		if(msg->CLD)
		{
			...
			//flag_steer_mode_ = false;
			shift_auto_ = false;
			std::cout << "Denger! Steer Clutch wire Expire" << std::endl;
			std::stringstream safety_error_message;
			safety_error_message << "Denger! Steer Clutch wire Expire";
			publishStatus(safety_error_message.str());
			can_send();
		}
		if(msg->PLD)
		{
			...
			//flag_steer_mode_ = false;
			shift_auto_ = false;
			std::cout << "Denger! Steer Potention wire Expire" << std::endl;
			std::stringstream safety_error_message;
			safety_error_message << "Denger! Steer Potention wire Expire";
			publishStatus(safety_error_message.str());
			can_send();
		}
		if(msg->JLD)
		{
			...
			//flag_steer_mode_ = false;
			shift_auto_ = false;
			std::cout << "Denger! Steer Joystick wire Expire" << std::endl;
			std::stringstream safety_error_message;
			safety_error_message << "Denger! Steer Joystick wire Expire";
			publishStatus(safety_error_message.str());
			can_send();
		}

		if(waypoint_param_.steer_deg != 0)
		{
			double th1 = waypoint_param_.steer_deg + setting_.steer_deg_th_left;
			double th2 = waypoint_param_.steer_deg + setting_.steer_deg_th_right;
			if(msg->angle_deg < th1 || msg->angle_deg > th2)
			{
				if(can_receive_501_.drive_auto == autoware_can_msgs::MicroBusCan501::STEER_AUTO)
					drive_clutch_ = false;
				if(can_receive_501_.steer_auto == autoware_can_msgs::MicroBusCan501::DRIVE_AUTO)
				...
				//flag_steer_mode_ = false;
				shift_auto_ = false;
				std::cout << "Denger! Steer deg th : " << msg->angle_deg << " , " << th1 << "," << th2 << std::endl;
				std::stringstream safety_error_message;
				safety_error_message << "Denger! Steer deg th : " << msg->angle_deg << " , " << th1 << "," << th2;
				publishStatus(safety_error_message.str());
				//system("aplay -D plughw:PCH /home/autoware/one33.wav");
				can_send();
			}
		...
		if(waypoint_param_.steer_deg_acc != 0)
		{
			double th1 = waypoint_param_.steer_deg_acc + setting_.steer_deg_acc_th_left;
			double th2 = waypoint_param_.steer_deg_acc + setting_.steer_deg_acc_th_right;
			if(msg->angle_deg_acc < th1 || msg->angle_deg_acc > th2)
			{
				if(can_receive_501_.drive_auto == autoware_can_msgs::MicroBusCan501::STEER_AUTO)
					drive_clutch_ = false;
				if(can_receive_501_.steer_auto == autoware_can_msgs::MicroBusCan501::DRIVE_AUTO)
				...
				//flag_steer_mode_ = false;
				shift_auto_ = false;
				std::cout << "Denger! Steer deg acc th : " << msg->angle_deg_acc << " , " << th1 << "," << th2 << std::endl;
				std::stringstream safety_error_message;
				safety_error_message << "Denger! Steer deg acc th : " << msg->angle_deg_acc << " , " << th1 << "," << th2;
				publishStatus(safety_error_message.str());
				//system("aplay -D plughw:PCH /home/autoware/one33.wav");
				can_send();
			}
	...
	void callbackMicrobusCan503(const autoware_can_msgs::MicroBusCan503::ConstPtr &msg)
	{
		...
		if(msg->MLD == true)
		{
			...
			//flag_steer_mode_ = false;
			shift_auto_ = false;
			std::cout << "Denger! Pedal Motor wire Expire" << std::endl;
			std::stringstream safety_error_message;
			safety_error_message << "Denger! Pedal Motor wire Expire";
			publishStatus(safety_error_message.str());
			can_send();
		}
		if(msg->CLD)
		{
			...
			//flag_steer_mode_ = false;
			shift_auto_ = false;
			std::cout << "Denger! Pedal Clutch wire Expire" << std::endl;
			std::stringstream safety_error_message;
			safety_error_message << "Denger! Pedal Clutch wire Expire";
			publishStatus(safety_error_message.str());
			can_send();
		}
		if(msg->PLD)
		{
			...
			//flag_steer_mode_ = false;
			shift_auto_ = false;
			std::cout << "Denger! Pedal Potention wire Expire" << std::endl;
			std::stringstream safety_error_message;
			safety_error_message << "Denger! Pedal Potention wire Expire";
			publishStatus(safety_error_message.str());
			can_send();
		}
		if(msg->JLD)
		{
			...
			//flag_steer_mode_ = false;
			shift_auto_ = false;
			std::cout << "Denger! Pedal Joystick wire Expire" << std::endl;
			std::stringstream safety_error_message;
			safety_error_message << "Denger! Pedal Joystick wire Expire";
			publishStatus(safety_error_message.str());
			can_send();
		}
						   ...
						   const geometry_msgs::PoseStampedConstPtr &pose_msg)
	{
		...
		double tire_angle;
		if(vehicle_cmd_.ctrl_cmd.steering_angle > 0) tire_angle = vehicle_cmd_.ctrl_cmd.steering_angle*wheelrad_to_steering_can_value_left + wheelrad_to_steering_can_value_left_intercept;
		else tire_angle = vehicle_cmd_.ctrl_cmd.steering_angle*wheelrad_to_steering_can_value_right + wheelrad_to_steering_can_value_right_intercept;
...
//		str << x << "," << v0 << "," << v << "," << v_sa;
		...
		name << "|" << "gnss_time";
		str <<  "|" << vehicle_cmd_.ctrl_cmd.linear_velocity * 3.6;
		name <<  "|" << "vehicle_cmd_.ctrl_cmd.linear_velocity_3.6";
		str <<  "|" << stopper_distance_flag_;
		name << "|" << "stopper_distance_flag";
		str <<  "|" << use_stopper_distance_val_for_brake_;
		name << "|" << "use_stopper_distance_val_for_brake";
		str <<  "|" << stopper_distance_fixed_velocity_for_brake_;
		name << "|" << "stopper_distance_fixed_velocity_for_brake";
		str <<  "|" << stopper_distance_send_process_for_brake_;
		name << "|" << "stopper_distance_send_process_for_brake";
		str <<  "|" << cruse_velocity_;
		name <<  "|" << "cruse_velocity";
		...
		name << "|" << "can_receive_501_.stroke_reply";
		...
		str << "|" << can_receive_503_.pedal_voltage_displacement;
		name << "|" << "can_receive_503_.pedal_voltage_displacement";
		str << "|" << can_receive_503_.pedal_can_displacement;
		name << "|" << "can_receive_503_.pedal_can_displacement";
		str << "|" << waypoint0_velocity_;
		name << "|" << "waypoint0_velocity_";
		str << "|" << (int)signal_takeover_flag_;
		name << "|" << "signal_takeover_flag_";
		str << "|" << rs232_shhv_arrange_.motor_current;
		name << "|" << "rs232_shhv_arrange_.motor_current";
		str << "|" << rs232_shhv_arrange_.motor_voltage;
		name << "|" << "rs232_shhv_arrange_.motor_voltage";
		str <<  "|" << acc2 ;
		name <<  "|" << "acc2" ;
		str <<  "|" << jurk2 ;
		name <<  "|" << "jurk2" ;
		str <<  "|" << cmd_steer_diff_ ;
		name <<  "|" << "cmd_steer_diff_" ;
		str <<  "|" << cmd_steer_diff_acc_ ;
		name <<  "|" << "cmd_steer_diff_acc_" ;
		str <<  "|" << vehicle_cmd_time_diff_ ;
		name <<  "|" << "vehicle_cmd_time_diff_" ;
		str <<  "|" << difference_toWaypoint_distance_.baselink_angular;
		name <<  "|" << "difference_toWaypoint_distance_.baselink_angular";
		str <<  "|" << difference_toWaypoint_distance_.baselink_distance;//11
		...
		str << "|" << routine_.data;
		name << "|" << "routine_.data";
		str << "|" << pid_params_.get_stroke_prev();
		name << "|" << "pid_params_.get_stroke_prev";
		str << "|" << pid_params_.get_stop_stroke_prev();
		name << "|" << "pid_params_.get_stop_stroke_prev";
		str << "|" << send_step_;//72
		name << "|" << "send_step";
	...
	void callbackTwistCmd(const autoware_msgs::VehicleCmd::ConstPtr &msg)
	{
		...
		std::cout <<"angle range : " << limitAngleData.limit_angle_bottom << "," << cmd_steer_diff_ << "," << limitAngleData.limit_angle_top << std::endl;
		std::cout << "VeicleCmd_time_diff : " << time_diff << std::endl;
		cmd_steer_diff_acc_ = cmd_steer_diff_ / time_diff;

		double deg_th_left, deg_th_right;
		if(zisoku <= setting_.steer_limit_function_select_th)
		{
			deg_th_left = setting_.steer_speed_limit1_left;
			deg_th_right = setting_.steer_speed_limit1_right;
		}
		else
		{
			deg_th_left = setting_.steer_speed_limit2_left;
			deg_th_right = setting_.steer_speed_limit2_right;
		}std::cout << "deg_th : " << deg_th_left << "," << deg_th_right << "," << cmd_steer_diff_acc_ << std::endl;
		if(cmd_steer_diff_acc_ < deg_th_left || cmd_steer_diff_acc_ > deg_th_right)
		{
			if(msg->ctrl_cmd.steering_angle != 0 && cmd_steer_diff_acc_ < 10000000 && cmd_steer_diff_acc_ > -10000000)
			{
				flag = true;
				angle_limit_over_ = true;
				std::cout << "Denger! target angle over1 : " << cmd_steer_diff_ << "," << cmd_steer_diff_acc_ << "," << time_diff << std::endl;
			}
		...
		double deg_th;
		if(zisoku <= 10) deg_th = setting_.steer_value_limit1;//100;
		else deg_th = setting_.steer_value_limit2;
		if(deg > deg_th && angle_limit_over_ == true) //2020_11_26修正 angle_limit_over_(角速度でエラー)がtrueの場合は処理しない
		{
			if(msg->ctrl_cmd.steering_angle != 0)
			{
				flag = true;
				angle_limit_over_ = true;
				std::cout << "Denger! target angle over2 : " << deg << std::endl;
			}
			else angle_limit_over_ = false;
		}
		else angle_limit_over_ = false;
		...
		if(flag == true && can_receive_502_.clutch == true && use_safety_localizer_ == true)
		{
			...
			//flag_steer_mode_ = false;
			shift_auto_ = false;
			safety_error_message << "target angle over3 , "<< deg << "," << cmd_steer_diff_ << "," << cmd_steer_diff_acc_ << "," << time_diff;
			//std::cout << safety_error_message.str() << std::endl;
			//system("aplay -D plughw:PCH /home/autoware/one33.wav");
			can_send();
		}

		if(msg->ctrl_cmd.steering_angle > vehicle_cmd_.ctrl_cmd.steering_angle) steer_direction_ = STEER_DIRECTION_LEFT;
		else steer_direction_ = STEER_DIRECTION_RIGHT;
		std::stringstream str;
		str << steer_direction_;
		std_msgs::String tmp;
		tmp.data = str.str();
		pub_tmp_.publish(tmp);

		vehicle_cmd_time_diff_ = time_diff;
		publishStatus(safety_error_message.str());
		vehicle_cmd_ = *msg;
	}
	...
	void callbackWaypoints(const autoware_msgs::Lane::ConstPtr &msg)
	{
		...
		std::cout << "waypoint_id : " << waypoint_id_ << std::endl;
		...
		waypoint_angle_ = yaw;
		waypoint0_velocity_ = msg->waypoints[0].twist.twist.linear.x;
		lane_pitch_ = msg->waypoints[1].waypoint_param.pitch;
		lane_pitch_average_ = msg->pitch_average;
	}
	...
	void bufset_steer(unsigned char *buf, ros::Duration time_diff)
	{
		double time_diff_val = time_diff.sec + time_diff.nsec * 1E-9;

		short steer_val;
		if(can_receive_501_.steer_auto != autoware_can_msgs::MicroBusCan501::STEER_AUTO) steer_val = 0;
		else if(input_steer_mode_ == false)
		{
			...
			if(wheel_ang > 0)
			{
				...
				/*if(difference_toWaypoint_distance_.baselink_distance > 0 && difference_toWaypoint_distance_.front_baselink_distance > 0 &&
					difference_toWaypoint_distance_.baselink_angular < 0 && steer_direction_ == STEER_DIRECTION_RIGHT)
				{
					steer_i_ += steer_i_own_second_value_ / time_diff_val;
					if(steer_i_max_ < steer_i_) steer_i_ = steer_i_max_;
				}
				steer_val += steer_i_;*/
			}
			else
			{
				...
				/*if(difference_toWaypoint_distance_.baselink_distance > 0 && difference_toWaypoint_distance_.front_baselink_distance > 0 &&
					difference_toWaypoint_distance_.baselink_angular > 0 && steer_direction_ == STEER_DIRECTION_LEFT)
				{
					steer_i_ += steer_i_own_second_value_ / time_diff_val;
					if(steer_i_max_ < steer_i_) steer_i_ = steer_i_max_;
				}
...
//P
		...
		//I
		double e_i;
		pid_params_.plus_steer_diff_sum_distance(e);

		if(current_velocity_.twist.linear.x < 1.0)
		{
			pid_params_.clear_steer_diff_distance();
		}

		double sum_diff = pid_params_.get_steer_diff_sum_distance();
		...
		//D
		double e_d = e - pid_params_.get_steer_e_prev_distance();
		...
		pid_params_.set_steer_e_prev_distance(e);
		return target_steer;
	}
	...
	double _brake_stroke_pid_control(double current_velocity_kmh, double cmd_velocity_kmh, double acceleration)
	{
		...
		double brake_stroke_adjust_th = (current_velocity_kmh + cmd_velocity_kmh)/2.0 * (setting_.brake_stroke_adjust_th / 100.0);
		...
		double stopper_distance_th = (setting_.stopper_distance1 > cmd_velocity_kmh*velocity_magn) ? setting_.stopper_distance1 : cmd_velocity_kmh*velocity_magn;
		if(pid_params_.get_stroke_prev() > 0 && (stopper_distance_.distance < 0 || stopper_distance_.distance > stopper_distance_th))
		{
			...
			if(current_velocity_kmh > cmd_velocity_kmh)
			{
				double stroke_kagen = math_stroke_kagen_brake(cmd_velocity_kmh);
		...
		//I
		double e_i, plus_e;
		if(stopper_distance_.fixed_velocity >= 0 || stopper_distance_.send_process == autoware_msgs::StopperDistance::UNKNOWN) plus_e = e;
		else
		{
			switch(stopper_distance_.send_process)
			{
	...
	void bufset_drive(unsigned char *buf, double current_velocity, double acceleration, double stroke_speed)
	{
		...
		if(can_receive_501_.drive_mode == autoware_can_msgs::MicroBusCan501::DRIVE_MODE_VELOCITY)
		{
			...
			{
				std::cout <<"jjj : " << vehicle_cmd_.ctrl_cmd.linear_velocity;
				double linearx = vehicle_cmd_.ctrl_cmd.linear_velocity;
		...
		else
		{
			/*double cmd_velocity;
			if(input_drive_mode_ == false)
				cmd_velocity = vehicle_cmd_.ctrl_cmd.linear_velocity * 3.6;
			else
			...
			{std::cout << "stroke stop" << std::endl;
				if(current_velocity < setting_.velocity_stop_th)
				{
					...
					pid_params_.set_stroke_state_mode_(PID_params::STROKE_STATE_MODE_STOP_);
				}
				else
				{
			...
			//ブレーキを離す場合、徐々に離す
			/*if(pid_params_.get_stroke_prev() < 0 && new_stroke >= 0)
			{
				double tmp = pid_params_.get_stroke_prev() + stroke_speed;
				if(tmp < new_stroke) new_stroke = tmp;
			}
			//ブレーキをゆっくり踏む
			if(pid_params_.get_stroke_prev() < 0.0 && pid_params_.get_stroke_prev() < new_stroke)
			{
				double tmp = pid_params_.get_stroke_prev() - stroke_speed;
				if(tmp > new_stroke) new_stroke = tmp;
				if(new_stroke < setting_.pedal_stroke_min) new_stroke = setting_.pedal_stroke_min;
			}
			pid_params_.set_stroke_prev(new_stroke);*/
...
public:
	kvaser_can_sender(ros::NodeHandle nh, ros::NodeHandle p_nh)
		...
		, stop_distance_over_sum_(0)
		, stopper_distance_flag_(0)
		, waypoint0_velocity_(0)
		, signal_takeover_flag_(false)
		, lane_pitch_(0)
		, use_distance_safety_(false)
		, steer_i_(0)
		, steer_i_own_second_value_(500)
		, steer_i_max_(500)
	{
		/*setting_.velocity_limit = 50;
		setting_.velocity_stop_th = 4.0;
		setting_.accel_max_i = 3000.0;
		setting_.brake_max_i1 = 500.0;
		...
		int kvaser_serial;
		private_nh_.param<int>("kvaser_serial", kvaser_serial, 0);
		...
		can_receive_501_.soot_combustion = false;
		canStatus res = kc.initSerial((unsigned int)kvaser_serial, canBITRATE_500K);
		...
		pub_way_distance_ave_ = nh_.advertise<std_msgs::String>("/microbus/way_distance_ave", 1, true);
		...
		sub_vehicle_cmd_ = nh_.subscribe("/vehicle_cmd", 10, &kvaser_can_sender::callbackTwistCmd, this);
		...
		sub_config_velocity_set_ = nh_.subscribe("/config/velocity_set_modification", 10, &kvaser_can_sender::callbackConfigVelocitySet, this);
		sub_config_temporary_stopper_ = nh_.subscribe("/config/temporary_stopper", 10, &kvaser_can_sender::callbackConfigTemporaryStopper, this);
		sub_config_lane_rule_ = nh_.subscribe("/config/lane_rule", 10, &kvaser_can_sender::callbackConfigLaneRule, this);
		...
		sub_solar_direction_ = nh.subscribe("/solar_direction", 10 , &kvaser_can_sender::callbackSolarDirection, this);
		sub_waypoints_file_name_ = nh.subscribe("/waypoints_file_name", 10 , &kvaser_can_sender::callbackWaypointFileName, this);
		sub_sip_signal_error_ = nh.subscribe("/sip/signal_error", 10 , &kvaser_can_sender::callbackSipSignalError, this);
		sub_rs232_sppm_ = nh.subscribe("/microbus/sppm", 10 , &kvaser_can_sender::callbackRS232SPPM, this);
		sub_rs232_shhv_ = nh.subscribe("/microbus/shhv", 10 , &kvaser_can_sender::callbackRS232SHHV, this);
		sub_rs232_shhv_arrange_ = nh.subscribe("/microbus/shhv_arrange", 10 , &kvaser_can_sender::callbackRS232SHHVArrange, this);
		sub_signal_takeover_ = nh.subscribe("/signal_takeover_flag", 10 , &kvaser_can_sender::callbackSignalTakeover, this);
		sub_node_dead_ = nh.subscribe("/node_dead", 10 , &kvaser_can_sender::callbackNodeDead, this);
		...
		way_pose_sum_.x = way_pose_sum_.y = way_pose_sum_.x = 0;
		way_pose_sum_ndt_.x = way_pose_sum_ndt_.y = way_pose_sum_ndt_.x = 0;
		way_pose_sum_gnss_.x = way_pose_sum_gnss_.y = way_pose_sum_gnss_.x = 0;
		way_pose_sum_ekf_.x = way_pose_sum_ekf_.y = way_pose_sum_ekf_.x = 0;
	}
	...
	void can_send()
	{
		ros::Time nowtime = ros::Time::now();//std::cout << "abc : " << nowtime.nsec << std::endl;
		ros::Duration time_diff = nowtime - run_time_;

		//if(can_receive_501_.emergency == false)
		{
			if(!current_pose_.header.stamp.isZero())// && (can_receive_502_.clutch == true || can_receive_503_.clutch == true))
			{
				ros::Duration time_diff = nowtime - current_pose_.header.stamp;
				ros::Duration time_limit(0.8);
				if(time_diff > time_limit)
				{
					...
					//flag_steer_mode_ = false;
					shift_auto_ = false;
					std::cout << "Denger! localizer time over : " << time_diff.sec << "," << time_diff.nsec << std::endl;
					std::stringstream safety_error_message;
					safety_error_message << "localizer time over\n" << current_pose_.header.stamp.sec << "," << current_pose_.header.stamp.nsec << std::endl;
					publishStatus(safety_error_message.str());
				}
			}
			NdtGnssCheck();
